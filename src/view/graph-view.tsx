...
export const GraphView = forwardRef<GraphViewHandle, GraphViewProps>(function GraphView(
  ...
  const projectionFnRef = useRef<ProjectionFn>((pos) => ({
    x: pos.x,
    y: pos.y,
    scale: 1,
    depth: 0,
  ...
  function rebuildProjection() {
    projectionFnRef.current =
      dimensionModeRef.current === '3d' ? make3DProjection() : make2DProjection()
  }
  ...
  function updateCameraAndRedraw() {
    if (dimensionModeRef.current === '2d') {
      ...
      if (svgContainer.current && d3Graphics.current) {
        ...
        d3Graphics.current.resize(
          svgContainer.current.clientWidth,
          svgContainer.current.clientHeight,
    ...
    zoomBy: (factor: number) => {
      if (dimensionModeRef.current === '3d') {
        cameraRef.current.distance = Math.max(
          100,
          Math.min(5000, cameraRef.current.distance / factor)
        )
        rebuildProjection()
      } else {
    ...
    const onMouseMove = (e: MouseEvent) => {
      if (panDragRef.current) {
        ...
        cameraRef.current.phi = Math.max(
          -Math.PI / 2 + 0.05,
          Math.min(Math.PI / 2 - 0.05, startPhi + dy * 0.005)
        )
        updateCameraAndRedraw()
      }
    }
      ...
      d3Graphics.current = new D3Graphics(
        ...
        onClickNode,
        {
          ...
          onNodeHover: (entity, x, y) => {
            ...
            setTooltip({
              content: entity.id,
              sub: group !== undefined ? `group ${group}` : undefined,
              x,
              y,
            })
          },
          onEdgeHover: (edge, x, y) => {
            const sub =
              edge.value !== undefined && edge.value > 1 ? `weight: ${edge.value}` : undefined
  ...
  const cursorStyle = panDragRef.current
    ? 'grabbing'
    : rotateDragRef.current
      ? 'all-scroll'
      : undefined
    ...
    <div className="relative h-full w-full">
      ...
      {tooltip && (
        ...
        >
          ...
          {tooltip.sub && <div className="text-[10px] text-accent/60 mt-0.5">{tooltip.sub}</div>}
        </div>
      )}
